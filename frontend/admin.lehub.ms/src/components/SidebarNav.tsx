...
import { communityPath, isSectionActive, PATHS, type CommunitySection } from '@/lib/navigation'
 ...
 *
 * La première dépend de la communauté active, la seconde n'appartient à personne — les
...
function NavEntry({ entry, collapsed }: { entry: Entry; collapsed: boolean }): ReactNode {
...
export function SidebarNav({ collapsed }: { collapsed: boolean }): ReactNode {
  ...
  /* La communauté de la **barre**, pas celle de l'écran. Les deux étaient confondues, et
     c'était le défaut : sur un écran d'administration générale, qui ne porte aucune communauté
     dans son URL, la section disparaissait et il ne restait aucun chemin de retour. */
