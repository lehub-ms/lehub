...
EOF
  ...
  # Chaque application renvoie vers l'autre : le backoffice pour la création de compte et
  # l'absence d'habilitation, le site public pour proposer le portail de gestion. Les deux
  # origines sont celles de ce slot.
  local public_site="http://$public_host:$LEHUB_WEB_PORT"
  local admin_site="http://$public_host:$LEHUB_ADMIN_PORT"

  _workspace_render_frontend_env frontend/lehub.ms "$api_origin" "$LEHUB_WEB_PORT" "$dev_host" \
    VITE_ADMIN_BASE_URL "$admin_site"
  _workspace_render_frontend_env frontend/admin.lehub.ms "$api_origin" "$LEHUB_ADMIN_PORT" "$dev_host" \
    VITE_PUBLIC_SITE_URL "$public_site"
...
# Les deux derniers arguments nomment et valorisent l'origine de *l'autre* application : le
# site public reçoit celle du backoffice, et réciproquement. Chaque application ne connaît que
# sa contrepartie, d'où deux noms de variable plutôt qu'un seul partagé.
_workspace_render_frontend_env() {
  local app="$1" api_origin="$2" port="$3" dev_host="$4" counterpart_var="${5:-}" counterpart="${6:-}"
  # Un `if` et non un `[[ … ]] && …` : sous `set -e`, ce dernier ferait sortir le script dès
  # que la contrepartie serait absente, ce qui est un cas légitime et non une erreur.
  local counterpart_line=''
  if [[ -n "$counterpart" ]]; then
    counterpart_line="$counterpart_var=$counterpart"
  fi
