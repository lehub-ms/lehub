...
EOF
  ...
  # Le backoffice renvoie vers le site public — création de compte, absence d'habilitation —
  # donc il a besoin de son origine, celle de ce slot.
  ...
  _workspace_render_frontend_env frontend/admin.lehub.ms "$api_origin" "$LEHUB_ADMIN_PORT" "$dev_host" "$public_site"
...
_workspace_render_frontend_env() {
  local app="$1" api_origin="$2" port="$3" dev_host="$4" public_site="${5:-}"
...
${public_site:+VITE_PUBLIC_SITE_URL=$public_site}
