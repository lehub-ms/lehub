...
EOF
  ...
  rendered="$(LEHUB_DB_NAME="$LEHUB_DB_NAME" MSSQL_SA_PASSWORD="$password" \
    LEHUB_CORS_ORIGINS="$cors" \
    node "$LIB_DIR/local-settings.mjs" \
      "$ROOT_DIR/api/local.settings.json" "$ROOT_DIR/api/local.settings.json.example")"

  _workspace_render_frontend_env frontend/lehub.ms "$api_origin" "$LEHUB_WEB_PORT"
  _workspace_render_frontend_env frontend/admin.lehub.ms "$api_origin" "$LEHUB_ADMIN_PORT"
...
_workspace_render_frontend_env() {
  local app="$1" api_origin="$2" port="$3"
