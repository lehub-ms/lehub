...
| Blobs uploaded through `POST /api/media/uploads` and never referenced stay for ever | the upload has to precede the entity being saved, because the edit panel previews the real URL — the alternative is a two-phase commit for a logo. A stray logo is a few kio against the 25 €/month cap, the account keeps 7-day soft delete, and `publicAccess: 'Blob'` means an orphan is unreachable without its exact generated name. A sweep would need an inventory of every referenced path across three tables plus a grace window for uploads in flight, so it is a background job rather than part of the route |
| Uploaded SVG icons are stored as-is, not rewritten | the route refuses a SVG carrying script, an `on*` handler, a `foreignObject`, a `DOCTYPE`, an external reference or a `javascript:` URL, rather than trying to sanitise one — a rewriter has to be right about every parser quirk for ever, a refusal only has to be right about what a logo needs. What survives that is served with `Content-Disposition: attachment`, which `<img>` ignores and a direct navigation honours, so the residual case of a document executing on the media origin is closed too. That origin holds no session in any event |
